...
describe('reopen after historical split', () => {
  it('uses the new BUY basis after the old position was closed', () => {
    const rows: Transaction[] = [
      ...
      { id: 'd', portfolio_id: 'p', symbol: 'XYZ', type: 'BUY', quantity: 5, price: 7, currency: 'USD', timestamp: '2026-01-04T00:00:00Z', created_at: '2026-01-04T00:00:00Z' },
    ];
    expect(calculatePositions(rows).positions[0]).toMatchObject({ quantity: 5, averageCost: 7, costBasis: 35, realizedPnL: 20 });
  });
});
