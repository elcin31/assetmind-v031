...
describe('cost basis after split and later buy', () => {
  it('adds only the later purchase cost to preserved basis', () => {
    ...
    expect(calculatePositions(rows).positions[0].costBasis).toBe(1600);
  });
});
