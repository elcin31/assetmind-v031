...
describe('split realized total invariant', () => {
  it('does not alter realized P&L accumulated before the split', () => {
    ...
    expect(result.positions[0].quantity).toBe(16);
    expect(result.positions[0].costBasis).toBe(80);
  });
});
