...
describe('split cash entry kind', () => {
  it('never emits SPLIT into cash ledger entries', () => {
    ...
    expect(buildCashLedger(rows, [], '2026-01-02T00:00:00Z', 'USD').entries.some((entry) => entry.kind === 'SPLIT')).toBe(false);
  });
});
