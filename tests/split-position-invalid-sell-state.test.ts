...
describe('invalid post-split sell state', () => {
  it('leaves the split-adjusted position intact', () => {
    const rows: Transaction[] = [
      ...
      { id: 'c', portfolio_id: 'p', symbol: 'XYZ', type: 'SELL', quantity: 21, price: 60, currency: 'USD', timestamp: '2026-01-03T00:00:00Z', created_at: '2026-01-03T00:00:00Z' },
    ];
    const result = calculatePositions(rows);
    expect(result.hadInvalidSell).toBe(true);
    expect(result.positions[0]).toMatchObject({ quantity: 20, averageCost: 50, costBasis: 1000 });
  });
});
