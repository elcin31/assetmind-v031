...
describe('partial fractional sell after split', () => {
  it('keeps fractional remaining basis', () => {
    const rows: Transaction[] = [
      ...
      { id: 'c', portfolio_id: 'p', symbol: 'XYZ', type: 'SELL', quantity: 0.5, price: 70, currency: 'USD', timestamp: '2026-01-03T00:00:00Z', created_at: '2026-01-03T00:00:00Z' },
    ];
    const result = calculatePositionsWithTotals(rows);
    expect(result.totalRealizedPnL).toBe(5);
    expect(result.positions[0]).toMatchObject({ quantity: 1, averageCost: 60, costBasis: 60 });
  });
});
