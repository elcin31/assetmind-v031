...
describe('split idempotency key', () => {
  it('preserves client_request_id from cloud', () => {
    const tx = splitFromCloudTransaction({
      ...
      recorded_at: '2026-01-01T00:00:01Z', created_at: null, client_request_id: 'request-123',
    });
    expect(tx.client_request_id).toBe('request-123');
  });
});
