...
describe('split persistence metadata', () => {
  it('adds source only to the cloud row', () => {
    ...
    expect('source' in tx).toBe(false);
    expect(splitToCloudTransaction(tx, 'u', 'p').source).toBe('assetmind-web');
  });
});
