...
describe('split ratio round trip', () => {
  it('preserves exact ratio parts', () => {
    const tx: SplitTransaction = { id: 's', portfolio_id: 'p', symbol: 'XYZ', type: 'SPLIT', split_numerator: 7, split_denominator: 3, currency: 'USD', timestamp: '2026-01-01T00:00:00Z', created_at: '2026-01-01T00:00:00Z' };
    ...
    if (restored.type !== 'SPLIT') throw new Error('Expected split');
    expect([restored.split_numerator, restored.split_denominator]).toEqual([7, 3]);
  });
});
