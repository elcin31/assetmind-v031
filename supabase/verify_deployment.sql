...
WHERE n.nspname = 'public' AND c.relname IN ('portfolios', 'transactions');
-- Expected: both rows, all rls_enabled = true.
...
-- Expected: one eight-argument function starting with UUID, UUID; false, false, false, true.

SELECT tablename, policyname, roles, cmd
FROM pg_policies
WHERE schemaname = 'public' AND tablename IN ('portfolios', 'transactions');
