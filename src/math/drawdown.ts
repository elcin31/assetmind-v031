...
export function drawdowns(series: DrawdownPoint[]) {
  if (
    ...
    series.some(
      (p, i) =>
        ...
        p.value < 0 ||
        (i > 0 && p.date <= series[i - 1].date),
    ) ||
    series[0].value <= 0
  )
    return null;
  let peak = series[0];
  let episode: DrawdownEpisode | null = null;
  ...
  for (const p of series) {
    ...
    if (p.value >= peak.value) {
      if (episode) {
        episode.recoveryDate = p.date;
        episode.duration = days(episode.startDate, p.date);
        episode.recoveryDuration = days(episode.bottomDate, p.date);
  ...
  return {
    ...
    currentDuration: episode?.duration ?? 0,
    recoveryTime:
      episodes.filter((e) => e.recoveryDuration !== null).at(-1)
        ?.recoveryDuration ?? null,
  };
}
