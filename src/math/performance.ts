  ...
  return {
    ...
    reason: complete
      ...
      : candidates.some((p) => p.traded)
        ? "Недоступно с текущей моделью операций: BUY/SELL не определяют внешние денежные потоки. Выберите период без сделок."
        : "Недостаточно непрерывной истории цен; отсутствующие интервалы не считаются нулевыми.",
  };
}
