...
export function correlation(a: number[], b: number[]): number | null {
  const cov = covariance(a, b); const va = covariance(a, a); const vb = covariance(b, b);
  const value = safeRatio(cov, va === null || vb === null || va <= EPSILON || vb <= EPSILON ? null : Math.sqrt(va * vb));
...
export function correlationMatrix(symbols: string[], histories: Map<string, HistoryBar[]>): CorrelationMatrix | null {
  if (!symbols.length || new Set(symbols).size !== symbols.length) return null;
  const series = symbols.map(s => datedReturns(histories.get(s) ?? []));
  const maps = series.map(rs => new Map(rs.map(r => [`${r.startDate}/${r.date}`, r])));
  ...
  if (cov.some(row => row.some(v => v === null)) || corr.some(row => row.some(v => v === null))) return null;
  return { symbols, returns, covariance: (cov as number[][]).map(row => row.map(v => v * 252)), correlation: corr as number[][], observations: keys.length };
}
export function averageCorrelation(matrix: number[][]): number | null {
  if (matrix.length < 2 || matrix.some(row => row.length !== matrix.length || row.some(v => !Number.isFinite(v) || Math.abs(v) > 1))) return null;
