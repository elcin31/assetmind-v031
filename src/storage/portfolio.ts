...
import { splitFromCloudTransaction, splitToCloudTransaction, type CloudSplitTransactionRow } from './splitTransaction';
...
interface CloudTradeTransactionRow {
  transaction_id: string; portfolio_id: string; type: 'BUY' | 'SELL'; symbol: string;
  quantity: number; price: number; currency: string; executed_at: string;
  recorded_at: string | null; created_at: string | null; client_request_id: string | null;
}
type CloudTransactionRow = CloudTradeTransactionRow | CloudSplitTransactionRow;
...
export function getPortfolioStorageKey(userId: string): string {
  ...
  if (!normalized || normalized.length > 128 || !/^[A-Za-z0-9_-]+$/.test(normalized)) throw new Error('A valid signed-in user is required to access portfolio data.');
  return `assetmind:${normalized}:portfolio.v1`;
}

function validTransaction(tx: Transaction, portfolio: Portfolio, ids: Set<string>, keys: Set<string>): boolean {
  if (!tx || !nonempty(tx.id) || ids.has(tx.id) || tx.portfolio_id !== portfolio.id || typeof tx.symbol !== 'string' || !/^[A-Z0-9.-]{1,20}$/.test(tx.symbol) || tx.currency !== portfolio.base_currency || !isDate(tx.timestamp) || !isDate(tx.created_at) || (tx.client_request_id !== undefined && (!nonempty(tx.client_request_id) || keys.has(tx.client_request_id)))) return false;
  if (tx.type === 'SPLIT') {
    return Number.isFinite(tx.split_numerator) && tx.split_numerator > 0 && Number.isFinite(tx.split_denominator) && tx.split_denominator > 0 && tx.split_numerator !== tx.split_denominator;
  }
  return Number.isFinite(tx.quantity) && tx.quantity > 0 && Number.isFinite(tx.price) && tx.price > 0;
}
...
export function validatePortfolio(value: unknown): LocalPortfolio {
  ...
  if (data.version !== 1 || !p || !nonempty(p.id) || !nonempty(p.name) || !/^[A-Z]{3}$/.test(p.base_currency) || !isDate(p.created_at) || !Array.isArray(data.transactions)) throw new Error('Unsupported or damaged portfolio data.');
  const ids = new Set<string>();
  const keys = new Set<string>();
  for (const tx of data.transactions) {
    if (!validTransaction(tx, p, ids, keys)) throw new Error('Invalid or duplicate transaction in portfolio data.');
    ids.add(tx.id);
...
function migrateLegacyPortfolio(storageKey: string): LocalPortfolio | null {
  ...
  try {
    ...
    try { localStorage.removeItem(storageKey); } catch { /* preserve original */ }
...
export function readPortfolio(userId: string): LocalPortfolio {
  ...
  return { version: 1, portfolio: { id: `local-${userId}`, name: 'My Portfolio', base_currency: 'USD', created_at: new Date().toISOString() }, transactions: [] };
}
...
function cloudMessage(error: { code?: string; message?: string } | null | undefined): string { return cloudErrorMessage(error, 'Cloud portfolio sync failed'); }
...
async function ensureCloudPortfolio(userId: string, local: LocalPortfolio): Promise<CloudPortfolioRow> {
  ...
  const existing = await supabase.from('portfolios').select('id,user_id,name,base_currency,created_at').eq('user_id', userId).maybeSingle();
  if (existing.error) throw new Error(cloudMessage(existing.error));
  if (existing.data) return existing.data as CloudPortfolioRow;
  const created = await supabase.from('portfolios').insert({ user_id: userId, name: local.portfolio.name, base_currency: local.portfolio.base_currency }).select('id,user_id,name,base_currency,created_at').single();
  if (!created.error && created.data) return created.data as CloudPortfolioRow;
  const retry = await supabase.from('portfolios').select('id,user_id,name,base_currency,created_at').eq('user_id', userId).maybeSingle();
...
function fromCloudTransaction(row: CloudTransactionRow): Transaction {
  if (row.type === 'SPLIT') return splitFromCloudTransaction(row);
  return { id: row.transaction_id, portfolio_id: row.portfolio_id, symbol: row.symbol.trim().toUpperCase(), type: row.type, quantity: Number(row.quantity), price: Number(row.price), currency: row.currency, timestamp: row.executed_at, created_at: row.recorded_at ?? row.created_at ?? row.executed_at, ...(row.client_request_id ? { client_request_id: row.client_request_id } : {}) };
}

async function migrateLocalPortfolioOnce(userId: string, local: LocalPortfolio, cloudPortfolio: CloudPortfolioRow) {
  if (!supabase) return;
  const settings = await supabase.from('user_settings').select('portfolio_migrated_at').eq('user_id', userId).maybeSingle();
  ...
  if (local.transactions.length > 0) {
    const existing = await supabase.from('transactions').select('transaction_id').eq('user_id', userId).eq('portfolio_id', cloudPortfolio.id);
    if (existing.error) throw new Error(cloudMessage(existing.error));
  ...
  const marked = await supabase.from('user_settings').upsert({ user_id: userId, portfolio_migrated_at: new Date().toISOString() }, { onConflict: 'user_id' });
...
async function loadCloudCanonical(userId: string): Promise<LocalPortfolio> {
  ...
  const rows = await supabase.from('transactions')
    .select('transaction_id,portfolio_id,type,symbol,quantity,price,currency,split_numerator,split_denominator,executed_at,recorded_at,created_at,client_request_id')
    .eq('user_id', userId).eq('portfolio_id', cloudPortfolio.id).in('type', ['BUY', 'SELL', 'SPLIT'])
  ...
  const canonical: LocalPortfolio = {
    version: 1,
    portfolio: { id: cloudPortfolio.id, name: cloudPortfolio.name, base_currency: cloudPortfolio.base_currency, created_at: cloudPortfolio.created_at },
...
function sameTransactionInput(existing: Transaction, input: NewTransaction): boolean {
  return existing.type !== 'SPLIT' && existing.symbol === input.symbol && existing.type === input.type && existing.quantity === input.quantity && existing.price === input.price && existing.currency === input.currency && existing.timestamp === input.timestamp;
}
...
export async function updateTransaction(transactionId: string, input: NewTransaction, userId: string) {
  return withStorageLock(userId, async () => {
    ...
    if (current.type === 'SPLIT') throw new Error('Stock split editing requires the dedicated corporate-action workflow.');
    const updated: TradeTransaction = { ...current, ...input, portfolio_id: data.portfolio.id };
    const transactions = [...data.transactions]; transactions[index] = updated;
    const next = validatePortfolio({ ...data, transactions });
    if (supabase) {
      const result = await supabase.from('transactions').update({ date: updated.timestamp.slice(0, 10), type: updated.type, symbol: updated.symbol, quantity: updated.quantity, price: updated.price, split_numerator: null, split_denominator: null, currency: updated.currency, executed_at: updated.timestamp }).eq('user_id', userId).eq('portfolio_id', data.portfolio.id).eq('transaction_id', transactionId).select('transaction_id').maybeSingle();
      requireCloudRow(result, 'Cloud portfolio sync failed', 'Transaction was not found in cloud. Refresh the portfolio before editing it.');
    }
    writePortfolio(next, userId);
    return updated;
  });
...
export async function deleteTransaction(transactionId: string, userId: string) {
  return withStorageLock(userId, async () => {
    ...
    try { next = validatePortfolio({ ...data, transactions }); }
    catch (error) {
      if (error instanceof Error && error.message.includes('SELL quantity')) throw new Error('This transaction cannot be deleted because a later SELL would exceed the available position. Edit the dependent transactions first.');
      throw error;
    }
    if (supabase) {
      const result = await supabase.from('transactions').delete().eq('user_id', userId).eq('portfolio_id', data.portfolio.id).eq('transaction_id', transactionId).select('transaction_id').maybeSingle();
      requireCloudRow(result, 'Cloud portfolio sync failed', 'Transaction was not found in cloud. Refresh the portfolio before deleting it.');
    }
    writePortfolio(next, userId);
  });
}
...
export async function importPortfolio(raw: string, userId: string) {
  ...
  return withStorageLock(userId, async () => {
    ...
    if (current.transactions.length > 0 && current.portfolio.id !== incoming.portfolio.id && incoming.portfolio.id !== `local-${userId}`) throw new Error('Backup belongs to another portfolio. Existing data was preserved.');
    const transactions = new Map(current.transactions.map(tx => [tx.id, tx]));
    for (const source of incoming.transactions) {
      const tx = { ...source, portfolio_id: current.portfolio.id } as Transaction;
      const existing = transactions.get(tx.id);
      if (existing && JSON.stringify(existing) !== JSON.stringify(tx)) throw new Error('Backup conflicts with an existing transaction. Nothing was changed.');
