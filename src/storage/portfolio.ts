...
export const STORAGE_KEY = 'assetmind.portfolio.v1';
...
export function readPortfolio(): LocalPortfolio {
  let raw: string | null;
  ...
  return { version: 1, portfolio: { id: 'local-portfolio', name: 'My Portfolio', base_currency: 'USD', created_at: new Date().toISOString() }, transactions: [] };
}

function writePortfolio(data: LocalPortfolio) {
...
async function withStorageLock<T>(action: () => T): Promise<T> {
...
export async function addTransaction(input: NewTransaction, requestId: string) {
  return withStorageLock(() => {
    ...
    // Millisecond tie-breaking stays chronological even for rapid writes.
    ...
    writePortfolio({ ...data, transactions: [...data.transactions, tx] });
    return tx;
  });
}
export function exportPortfolio() {
  const raw = localStorage.getItem(STORAGE_KEY) ?? JSON.stringify(readPortfolio());
  const url = URL.createObjectURL(new Blob([raw], { type: 'application/json' }));
  const a = document.createElement('a'); a.href = url; a.download = 'assetmind-backup.json'; a.click();
  setTimeout(() => URL.revokeObjectURL(url), 1000);
}
export async function importPortfolio(raw: string) {
  const incoming = validatePortfolio(JSON.parse(raw));
  return withStorageLock(() => {
    const current = readPortfolio();
    if (current.transactions.length === 0) { writePortfolio(incoming); return; }
    ...
    writePortfolio({ ...current, transactions: [...transactions.values()] });
  });
}
