  ...
  return <>
    ...
    <div className="workspace">
      ...
      <main className="main-content">
        <header className="header"><div><span className="eyebrow">ВАШЕ ФИНАНСОВОЕ ПРОСТРАНСТВО</span><h1>{tabs.find(t => t.id === tab)?.label}</h1><p className="header-sub">{s.portfolio.name} · {s.portfolio.base_currency}</p></div><div className="header-actions"><button className="btn btn-ghost" onClick={onRefresh} disabled={loading || signingOut}>{loading ? 'Обновление…' : '↻ Обновить'}</button><button className="btn btn-ghost" onClick={() => void handleSignOut()} disabled={signingOut}>{signingOut ? 'Выход…' : 'Выйти'}</button></div></header>
