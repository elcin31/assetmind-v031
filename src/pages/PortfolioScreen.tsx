  ...
  return <>
    ...
    <div className="workspace">
      ...
      <main className="main-content">
        ...
        {controller.coverageNotices.length > 0 && <details className="formula"><summary>Покрытие исторических данных</summary><p className="caption">{controller.coverageNotices.join(' ')}</p></details>}
        {controller.errors.length > 0 && <div className="notice" role="status">{controller.historyState === "provider_unavailable" ? "Поставщик истории недоступен: " : "История загружена частично: "}{controller.errors.join('; ')}. <button className="text-button" onClick={controller.retry}>Повторить загрузку</button></div>}
        {tab === 'overview' && <>
          <section className="portfolio-hero"><div><span className="eyebrow">{s.valuation.complete ? 'СТОИМОСТЬ АКТИВОВ' : 'СЕБЕСТОИМОСТЬ ПОЗИЦИЙ'}</span><div className="hero-value">{money(s.valuation.complete ? s.portfolioValue : cost)}</div><p>{s.valuation.complete ? 'Рыночная оценка открытых позиций' : 'Рыночная оценка неполная: ожидаем котировки'}</p><div className="hero-performance"><span>Доходность периода <b>{pct(analytics.performance.totalReturn)}</b></span><span>P&amp;L за всё время <b>{s.valuation.complete ? money(s.totalPnL) : '—'}</b></span></div><button className="btn btn-primary" onClick={() => setTab('trade')}>＋ Добавить сделку</button></div><div className="hero-aside"><span className="orbit" aria-hidden="true">a</span><div><b>{s.positions.length}</b><span>активов</span><b>{s.transactions.length}</b><span>сделок</span></div></div></section>
