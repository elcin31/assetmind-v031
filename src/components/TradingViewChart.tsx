...
export function TradingViewChart({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const timer = window.setTimeout(() => {
        if (active && !wrapper.querySelector('iframe')) setFailed(true);
      }, 15000);
  ...
  return (
    <div className="chart-fallback">
      <p className="caption">
        График TradingView · данные могут поступать с задержкой.
      </p>
      <div
        ref={host}
        className="tradingview-host"
        hidden={failed}
        aria-label={`График TradingView: ${symbol}`}
      />
      {failed && (
        <p className="empty" role="status">
          Не удалось загрузить график. Проверьте соединение или откройте его по
          ссылке ниже.
        </p>
      )}
      <a
