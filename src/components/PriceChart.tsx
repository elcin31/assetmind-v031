...
export function PriceChart({ symbol }: { symbol: string }) {
  ...
  useEffect(() => {
    let active = true;
    void loadHistory(symbol, period)
      .then((bars) => {
  ...
  return (
    <div className="price-chart" aria-busy={!current}>
      ...
      {!current ? (
        ...
        <div>
          ...
          <TradingViewChart key={requestKey} symbol={symbol} period={period} />
          <button
            type="button"
        ...
        <div>
          <p className="caption" role="status">
            Недостаточно дневных цен за этот период.
          </p>
          <TradingViewChart key={requestKey} symbol={symbol} period={period} />
        </div>
      ) : (
        <>
          <div className="chart-readout">
            <strong>{number(focused.close)}</strong>
            <span>{date(focused.date)}</span>
          </div>
          <svg
            className="price-plot"
            viewBox="0 0 600 230"
            role="img"
            aria-label={`История adjusted close ${symbol}, от ${date(first.date)} до ${date(last.date)}. Изменение ${change.toFixed(2)} процентов.`}
            onPointerMove={(event) => {
              ...
              for (let i = 1; i < points.length; i++)
                if (
                  Math.abs(points[i].x - x) < Math.abs(points[nearest].x - x)
                )
                  nearest = i;
              setCursor(nearest);
            }}
          ...
          >
            ...
            <polygon
              points={`${points[0].x},214 ${line} ${points[points.length - 1].x},214`}
              fill={`url(#${gradient})`}
            />
            <polyline
              points={line}
            ...
            <circle
              ...
              strokeWidth="2"
            />
            <text x="20" y="13" fill="var(--text-muted)" fontSize="11">
              {number(max)}
            </text>
            <text x="20" y="227" fill="var(--text-muted)" fontSize="11">
              {number(min)}
            </text>
          </svg>
          ...
          <input
            ...
            max={bars.length - 1}
            value={index}
            aria-label="Дата на графике"
            aria-valuetext={`${date(focused.date)}: ${number(focused.close)}`}
            onChange={(event) => setCursor(Number(event.target.value))}
          />
          <p className="caption">
            Дневной adjusted close в валюте котировки. Последняя дата:{' '}
            {date(last.date)}. Источник history: server-side Yahoo Chart;
            quote/search остаются на Finnhub.
          </p>
        </>
      )}
    </div>
